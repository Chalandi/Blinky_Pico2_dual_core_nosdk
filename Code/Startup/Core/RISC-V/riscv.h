...
#define _riscv_read_csr(csrname) ({ \
    ...
    __asm volatile ("csrr %0, " #csrname : "=r" (__csr_tmp_u32)); \
    __csr_tmp_u32; \
})

#define _riscv_write_csr(csrname, data) ({ \
    if (__builtin_constant_p(data) && !((data) & -32u)) { \
        __asm volatile ("csrwi " #csrname ", %0" : : "i" (data)); \
    } else { \
        __asm volatile ("csrw " #csrname ", %0" : : "r" (data)); \
    } \
})

#define _riscv_set_csr(csrname, data) ({ \
    if (__builtin_constant_p(data) && !((data) & -32u)) { \
        __asm volatile ("csrsi " #csrname ", %0" : : "i" (data)); \
    } else { \
        __asm volatile ("csrs " #csrname ", %0" : : "r" (data)); \
    } \
})

#define _riscv_clear_csr(csrname, data) ({ \
    if (__builtin_constant_p(data) && !((data) & -32u)) { \
        __asm volatile ("csrci " #csrname ", %0" : : "i" (data)); \
    } else { \
        __asm volatile ("csrc " #csrname ", %0" : : "r" (data)); \
    } \
})
...
#define _riscv_read_write_csr(csrname, data) ({ \
    uint32_t __csr_tmp_u32; \
    if (__builtin_constant_p(data) && !((data) & -32u)) { \
        __asm volatile ("csrrwi %0, " #csrname ", %1": "=r" (__csr_tmp_u32) : "i" (data)); \
    } else { \
        __asm volatile ("csrrw %0, " #csrname ", %1": "=r" (__csr_tmp_u32) : "r" (data)); \
    } \
    __csr_tmp_u32; \
})
...
#define _riscv_read_set_csr(csrname, data) ({ \
    uint32_t __csr_tmp_u32; \
    if (__builtin_constant_p(data) && !((data) & -32u)) { \
        __asm volatile ("csrrsi %0, " #csrname ", %1": "=r" (__csr_tmp_u32) : "i" (data)); \
    } else { \
        __asm volatile ("csrrs %0, " #csrname ", %1": "=r" (__csr_tmp_u32) : "r" (data)); \
    } \
    __csr_tmp_u32; \
})
...
#define _riscv_read_clear_csr(csrname, data) ({ \
    uint32_t __csr_tmp_u32; \
    if (__builtin_constant_p(data) && !((data) & -32u)) { \
        __asm volatile ("csrrci %0, " #csrname ", %1": "=r" (__csr_tmp_u32) : "i" (data)); \
    } else { \
        __asm volatile ("csrrc %0, " #csrname ", %1": "=r" (__csr_tmp_u32) : "r" (data)); \
    } \
    __csr_tmp_u32; \
})
